...
int Normal2dEstimation::searchForNeighbors(int index, std::vector<int>& nn_indices,
  ...
  if (m_in_cloud->is_dense) {
    ...
    for (unsigned int idx = 0; idx < m_indices->size(); ++idx) {
      if (this->searchForNeighbors((*m_indices)[idx], *nn_indices, nn_dists) == 0) {
        normal_cloud->points[idx].x = normal_cloud->points[idx].y = normal_cloud->points[idx].z =
            std::numeric_limits<float>::quiet_NaN();
        normal_cloud->is_dense = false;
        continue;
      }

      this->computePointNormal2d(nn_indices, normal_cloud->points[idx].x,
                                 normal_cloud->points[idx].y, normal_cloud->points[idx].z);
    ...
    for (unsigned int idx = 0; idx < m_indices->size(); ++idx) {
      if (this->searchForNeighbors((*m_indices)[idx], *nn_indices, nn_dists) == 0) {
        normal_cloud->points[idx].normal_x = normal_cloud->points[idx].normal_y =
            normal_cloud->points[idx].normal_z = std::numeric_limits<float>::quiet_NaN();
        normal_cloud->is_dense = false;
        continue;
      }

      this->computePointNormal2d(
          nn_indices, normal_cloud->points[idx].normal_x, normal_cloud->points[idx].normal_y,
          normal_cloud->points[idx].normal_z, normal_cloud->points[idx].curvature);

      this->flipNormalTowardsViewpoint(
          m_in_cloud->points[(*m_indices)[idx]], normal_cloud->points[idx].normal_x,
          normal_cloud->points[idx].normal_y, normal_cloud->points[idx].normal_z);
    }
  } else {
    // Iterating over the entire index vector
    for (unsigned int idx = 0; idx < m_indices->size(); ++idx) {
...
void Normal2dEstimation::flipNormalTowardsViewpoint(const Point& point, float& x, float& y,
                                                    float& z) const {
...
void Normal2dEstimation::flipNormalTowardsViewpoint(const Point& point,
                                                    Eigen::Matrix<double, 3, 1>& normal) const {
  Eigen::Matrix<double, 3, 1> vp(m_view_point.x - point.x, m_view_point.y - point.y, 0.);
