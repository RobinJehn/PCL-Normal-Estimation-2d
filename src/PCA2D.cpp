...
void PCA2D::initCompute() {
  ...
  Eigen::Matrix2f alpha = static_cast<Eigen::Matrix2f>(cloud_demean.topRows<2>() *
                                                       cloud_demean.topRows<2>().transpose());
...
void PCA2D::project(const ConstPtrCloud& in_cloud, PtrCloud& out_cloud) {
  if (in_cloud->is_dense) {
    ...
    pcl::PointXYZ p;
    for (size_t i = 0; i < in_cloud->size(); ++i) {
      if (!std::isfinite(in_cloud->points[i].x) || !std::isfinite(in_cloud->points[i].y) ||
