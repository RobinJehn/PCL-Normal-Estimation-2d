...
/** \brief Normal2dEstimation estimate local straight line at each points using it neighbour.
 * Contrary to native normal estimation  on PCL lib, it works only on 2D points cloud.
 *
 * \note This class use 3D points cloud but ignore all z components. So make sure you have all your
 * points on z=0 plan.
 *
 *
 */
class Normal2dEstimation {
 public:
   ...
   */
  Normal2dEstimation(double search_radius = 0.0, unsigned int k = 0)
   ...
   *
   *  The view points is used in normal orientation purpose. This orientation force all normals to
   * look at view points.
   *
 ...
 private:
  /** \brief Search for nearest neighbors using the spatial locator from search class and class
   * parameters (k and search_radius).
   *
   ...
   * \param[out] nn_dists the resultant vector of distances representing the distances from the
   * query point to the neighbors.
   *
   * \return The number of neighbors found. If no neighbors are found or an error occurred, return
   * 0.
   */
  int searchForNeighbors(int index, std::vector<int>& nn_indices,
                         std::vector<float>& nn_dists) const;

  /** \brief Compute the Least-Squares straight line fit for a given set of points, using their
    indices,
    ...
    */
  bool computePointNormal2d(boost::shared_ptr<std::vector<int>>& indices, float& nx, float& ny,
                            float& nz) const;

  /** \brief Compute the Least-Squares straight line fit for a given set of points, using their
   * indices, and return the estimated normal parameters.
   *
   * \param indices the point cloud indices that need to be used.
   * \param nx the resultant X component of the plane normal.
   * \param ny the resultant Y component of the plane normal.
  ...
  double m_search_radius;  ///< Max distance between the estimate normal position with each
                           ///< neighbour for take them on normal estimation.
